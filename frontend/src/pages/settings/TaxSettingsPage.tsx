      ...
      <div className={`${sectionCard} overflow-hidden anim-fade-up delay-100`}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold font-display text-white hover:brightness-95"
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button onClick={handleSaveTaxGroup} disabled={saving} className="px-4 py-2 rounded-xl text-sm font-semibold text-white disabled:opacity-60 hover:brightness-95" style={{ background: 'var(--accent)' }}>
              {saving ? 'Saving...' : 'Save'}
            </button>
          </>
        }
      >
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={taxForm.name} onChange={(e) => setTaxForm((f) => ({ ...f, name: e.target.value }))} placeholder="e.g. GST 5%" className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
          </div>

          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-medium text-slate-700 font-display">Components</label>
              <button type="button" onClick={addComponent} className="text-xs text-red-600 hover:underline font-display">+ Add component</button>
            </div>
            <div className="space-y-2">
              {taxForm.components.map((c, i) => (
                <div key={i} className="flex gap-2 items-center">
                  <select value={c.type} onChange={(e) => updateComponent(i, 'type', e.target.value)} className="flex-1 px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500">
                    {COMPONENT_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
                  </select>
                  <div className="flex items-center gap-1 w-24">
                    <input type="number" value={c.rate} onChange={(e) => updateComponent(i, 'rate', e.target.value)} min="0" max="100" step="0.5" className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button onClick={handleSaveCharge} disabled={saving} className="px-4 py-2 rounded-xl text-sm font-semibold text-white disabled:opacity-60 hover:brightness-95" style={{ background: 'var(--accent)' }}>
      ...
      >
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={chargeForm.name} onChange={(e) => setChargeForm((f) => ({ ...f, name: e.target.value }))} placeholder="e.g. Service Charge" className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5 font-display">Type</label>
              <select value={chargeForm.type} onChange={(e) => setChargeForm((f) => ({ ...f, type: e.target.value }))} className="w-full px-3 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500">
            ...
            <div>
              ...
              <input type="number" value={chargeForm.value} onChange={(e) => setChargeForm((f) => ({ ...f, value: e.target.value }))} className="w-full px-3 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
            </div>
          </div>
